...
class Conv1D(fluid.dygraph.Layer):
    def __init__(self, name_scope,
        ...
        self._conv2d = fluid.dygraph.Conv2D(self.full_name(),
...
class BMN(fluid.dygraph.Layer):
    def __init__(self, name_scope, cfg):
        ...
        # Base Module
        self.b_conv1 = Conv1D(name_scope="Base_1",
                              input_size=400,
                              ...
                              groups=4,
                              act="relu")
        self.b_conv2 = Conv1D(name_scope="Base_2",
                              num_filters=self.hidden_dim_1d,
                              size_k=3,
                              padding=1,
                              groups=4,
        ...
        self.ts_conv1 = Conv1D(name_scope="TEM_s1",
                               num_filters=self.hidden_dim_1d,
                               size_k=3,
                               padding=1,
                               groups=4,
                               act="relu")
        self.ts_conv2 = Conv1D(name_scope="TEM_s2",
                               num_filters=1,
                               size_k=1,
                               padding=0,
                               act="sigmoid")
        self.te_conv1 = Conv1D(name_scope="TEM_e1",
                               num_filters=self.hidden_dim_1d,
                               size_k=3,
                               padding=1,
                               groups=4,
                               act="relu")
        self.te_conv2 = Conv1D(name_scope="TEM_e2",
                               num_filters=1,
                               size_k=1,
                               padding=0,
                               act="sigmoid")
        ...
        self.p_conv1 = Conv1D(name_scope="PEM_1d",
        ...
        sample_mask = get_interp1d_mask(
            self.tscale, self.dscale, self.prop_boundary_ratio, self.num_sample, self.num_sample_perbin)
        self.sample_mask = fluid.dygraph.base.to_variable(sample_mask)
        self.sample_mask.stop_gradient = True

        self.p_conv3d1 = fluid.dygraph.Conv3D(name_scope="PEM_3d1",
        ...
        self.p_conv2d1 = fluid.dygraph.Conv2D(name_scope="PEM_2d1",
                                              num_filters=self.hidden_dim_2d,
                                              filter_size=1,
                                              stride=1,
                                              padding=0,
                                              act="relu")
        self.p_conv2d2 = fluid.dygraph.Conv2D(name_scope="PEM_2d2",
                                              num_filters=self.hidden_dim_2d,
                                              filter_size=3,
                                              stride=1,
                                              padding=1,
                                              act="relu")
        self.p_conv2d3 = fluid.dygraph.Conv2D(name_scope="PEM_2d3",
                                              num_filters=self.hidden_dim_2d,
                                              filter_size=3,
                                              stride=1,
                                              padding=1,
                                              act="relu")
        self.p_conv2d4 = fluid.dygraph.Conv2D(name_scope="PEM_2d4",
                                              num_filters=2,
                                              filter_size=1,
                                              stride=1,
                                              padding=0,
    ...
    def forward(self, x):
        ...
        xp = fluid.layers.reshape(xp, shape=[0, 0, -1, self.dscale, self.tscale])
...
def bmn_loss_func(pred_bm, pred_start, pred_end, gt_iou_map, gt_start,
                  gt_end, cfg):
    def _get_mask(cfg):
...
def val_bmn(model, config, args):
    ...
    for batch_id, data in enumerate(val_reader()):
        ...
        loss, tem_loss, pem_reg_loss, pem_cls_loss = bmn_loss_func(pred_bm, pred_start, pred_end, gt_iou_map, gt_start,
                                                                   gt_end, config)
...
def train_bmn(args):
	...
	reader = BMNReader(mode="train", cfg=train_config)
        ...
        for epoch in range(args.epoch):
            ...
            save_model_name = os.path.join(args.save_dir, "bmn_paddle_dy" + "_epoch{}".format(epoch))
        ...
        if fluid.dygraph.parallel.Env().local_rank == 0:
            save_model_name = os.path.join(args.save_dir, "bmn_paddle_dy" + "_final")
            fluid.dygraph.save_dygraph(bmn.state_dict(), save_model_name)
