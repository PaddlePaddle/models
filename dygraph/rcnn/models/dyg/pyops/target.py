...
@jit
def rpn_target_assign(anchor_box,
                      gt_boxes,
                      is_crowd,
                      im_info,
                      rpn_straddle_thresh,
...
def generate_proposal_labels(rpn_rois,
    ...
    o_labels = np.concatenate(
        labels_int32, axis=0).astype(np.int32).reshape(-1, 1)
    o_bbox_targets = np.concatenate(bbox_targets, axis=0).astype(np.float32)
    o_bbox_inside_weights = np.concatenate(
        bbox_inside_weights, axis=0).astype(np.float32)
    o_bbox_outside_weights = np.concatenate(
        bbox_outside_weights, axis=0).astype(np.float32)
    o_rois_lod = np.asarray(rois_lod, np.int32)

    return o_rois, o_labels, o_bbox_targets, o_bbox_inside_weights, o_bbox_outside_weights, o_rois_lod
...
def generate_mask_labels(im_info, gt_classes, is_crowd, gt_segms, rois,
    ...
    #print("mask rois: ", np.abs(o_mask_rois).mean(), o_mask_rois.shape)
    #print("roi_has_mask: ", np.abs(o_roi_has_mask_int32).mean(), o_roi_has_mask_int32.shape)
    #print("mask labels: ", np.abs(o_mask_int32).mean(), o_mask_int32.shape)
