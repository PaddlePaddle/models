...
def relative_err(a,b,real=True):
    ...
    if real:
        ...
        err = np.sum((a.real-b.real)**2) / (EPS+np.sum(a.real**2)+np.sum(b.real**2))
        err += np.sum((a.imag-b.imag)**2) / (EPS+np.sum(a.imag**2)+np.sum(b.imag**2))
...
def test_melspectrogram():
    a = pa.melspectrogram(x,window_size=512,
                            sample_rate=16000, 
                            hop_length=320,
                            n_mels=64,
                            fmin=50,
    ...
    b = librosa.feature.melspectrogram(x,sr=16000,n_fft=512, 
                                          win_length=512,
                                          hop_length=320, 
                                          n_mels=64, fmin=50)
...
def test_mfcc():
    ...
    a = pa.mfcc(x,
        ...
        lifter=0,
        **kwargs)
    S = librosa.feature.melspectrogram(x,sr=16000,n_fft=512, 
                                              win_length=512,
                                              hop_length=320, 
                                              n_mels=64, fmin=50)
