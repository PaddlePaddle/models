...
if __name__=='__main__':
    ...
    if args.restore!=-1:
       ...
       # for p in model.parameters():
        ...
        model = ModelClass(pretrained=True,num_classes=c['num_classes'],
                           drop_rate=c['dropout']) # use imagenet pretrained 
        optimizer = Adam(learning_rate=c['start_lr'],
                                          parameters=model.parameters())
        start_epoch = 0
    ...
    if args.restore!=-1:
        val_acc,val_preci,val_recall,mAP_scores = evaluate(args.restore,val_loader,
                                                               model,bce_loss,log_writer)
    ...
    for epoch in range(start_poch,epoch_num):
        ...
        for batch_id, data in enumerate(train_loader()):
            ...
            if step % c['checkpoint_step'] ==0 and local_rank==0:
                ...
                val_acc,val_preci,val_recall,mAP_scores = evaluate(epoch,val_loader,
                                                               model,bce_loss,log_writer)
                avg_map = np.mean(mAP_scores)
                ...
                if avg_map > best_mAP:
                    ...
                    fn = os.path.join(c['model_path'],'{}_epoch{}_mAP{:.3}_preci{:.3}_recall{:.3}.pdparams'.format(prefix,epoch,avg_map,val_preci,val_recall))
