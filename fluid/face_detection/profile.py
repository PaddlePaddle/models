...
add_arg('skip_reader',      bool,  True,            "Whether to skip data reader.")
...
def train(args, config, train_file_list, optimizer_method):
    ...
    reader_iter = train_reader()
    data = reader_iter.next()
    ...
    def run(iterations, feed_data):
        ...
        for batch_id in range(iterations):
            ...
            if not skip_reader:
                feed_data = feeder.feed(reader_iter.next())
            end_time = time.time()
    ...
    # profiling
    start = time.time()
    with profiler.profiler('All', 'total', '/tmp/profile_parallel_do'):
