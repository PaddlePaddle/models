...
class Settings(object):
    def __init__(self, data_dir, resize_h, resize_w, mean_value):
        ...
        self._img_mean = np.array(mean_value)[:, np.newaxis, np.newaxis].astype('float32')
        ...
        with open(file_list) as flist:
            ...
            for line in lines:
                if mode == 'train' or mode == 'test':
                    ...
                    img_label = cv2.resize(img_label, (settings.resize_w, settings.resize_h), interpolation = cv2.INTER_NEAREST)
                elif mode == 'infer':
