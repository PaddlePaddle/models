...
def main():
    ...
    trainval_file = os.path.join(devkit_dir, 'VOC2012', 'ImageSets', 'Segmentation', 'trainval.txt')
    ...
    test_file = os.path.join(devkit_dir, 'VOC2007', 'ImageSets', 'Segmentation', 'test.txt')
    
    with open(trainval_file, 'r') as input_f:
        for line in input_f:
            img = cv2.imread(os.path.join(segclass_dir, '%s.png' % line.strip()))
