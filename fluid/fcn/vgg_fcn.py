...
def vgg16_fcn(input, args):
    ...
    '''
    def conv(input, k_h, k_w, c_o, s_h, s_w, name, relu=True, padding=None, group=1, biased=True):
        if padding is None:
            padding = [0, 0]
        ...
        output = fluid.layers.conv2d(
            ...
            padding=padding,
            groups=group,
            param_attr=fluid.ParamAttr(name=prefix + 'weights', initializer = fluid.initializer.Xavier()),
            bias_attr=fluid.ParamAttr(name=prefix + 'biases', initializer = fluid.initializer.Constant(0)),
    ...
    if args.fcn_arch == 'fcn-32s': 
        upscore = fluid.layers.conv2d_transpose(
            ...
            filter_size=156, 
            stride=16,
            param_attr=fluid.ParamAttr(name='upscore_weights', initializer=fluid.initializer.Xavier()),
            bias_attr=fluid.ParamAttr(name='upscore_biases', initializer=fluid.initializer.Constant(0)))   
        ...
        upscore2 = fluid.layers.conv2d_transpose(
            ...
            filter_size=5, 
            stride=3,
            param_attr=fluid.ParamAttr(name='upscore2_weights', initializer=fluid.initializer.Xavier()),
            bias_attr=fluid.ParamAttr(name='upscore2_biases', initializer=fluid.initializer.Constant(0)))
        ...
        upscore1 = fluid.layers.conv2d_transpose(
            ...
            filter_size=1, 
            stride=2,
            param_attr=fluid.ParamAttr(name='upscore1_weights', initializer=fluid.initializer.Xavier()),
            bias_attr=fluid.ParamAttr(name='upscore1_biases', initializer=fluid.initializer.Constant(0)))
        score_pool3 = conv(pool3, 1, 1, 21, 1, 1, name='score_pool3')
        #fuse_pool3 = upscore1 + score_pool3
