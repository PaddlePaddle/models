...
def eval(args):
    ...
    if loss_name == "tripletloss":
        ...
        metricloss = emlloss(test_batch_size = args.batch_size, samples_each_class=2, fea_dim=2048)
        cost = metricloss.loss(out[0])
    ...
    for batch_id, (data, label) in enumerate(test_reader()):
        t1 = time.time()
        loss, feas = exe.run(test_program, fetch_list = fetch_list, feed=feeder.feed(data))
        f.append(feas)
        l.append(label)
