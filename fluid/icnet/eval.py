...
def create_iou(predict, label, mask, num_classes, image_shape):
    predict = fluid.layers.upsampling_bilinear2d(predict, out_shape=image_shape[1:3])
...
def eval(args):
    ...
    iou, out_w, out_r = create_iou(sub124_out, label, mask, num_classes, data_shape)
    ...
    for data in test_reader():
        count += 1
        result = exe.run(feed=get_feeder_data(data, place, for_test=True),
                             fetch_list=fetch_vars)
        out_wrong += result[1]
        out_right += result[2]
        print "count: %s; current iou: %.3f;" % (count, result[0])
        sys.stdout.flush()
    iou = cal_mean_iou(out_wrong, out_right, num_classes)
    print "\nmean iou: %.3f" % iou
