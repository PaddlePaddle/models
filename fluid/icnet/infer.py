...
def color(input):
    ...
    for i in input.flatten():
        result.append([label_colours[i][2], label_colours[i][1], label_colours[i][0]])
...
def infer(args):
    ...
    predict = fluid.layers.reshape(predict, shape=[data_shape[1], data_shape[2], -1]) # batch_size should be 1
    ...
    for line in open(args.images_list):
        ...
        image = paddle.image.load_image(image_file, is_color=True).astype("float32")
        image -= IMG_MEAN
