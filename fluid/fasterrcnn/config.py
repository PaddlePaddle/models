...
# --------------------------------------------------------
"""Faster RCNN config system.

This file specifies default config options for Faster RCNN. You should not
...
Faster RCNN supports a lot of different model types, each of which has a lot of
...
_RENAMED_KEYS = {
    ...
    ('TRAIN.PROPOSAL_FILES', "Also convert to a tuple, e.g., " +
     ...
     "'path/to/file1:path/to/file2' -> " + "('path/to/file1', 'path/to/file2')"
     ),
    'TEST.SCALES':
    ('TEST.SCALE',
...
# ---------------------------------------------------------------------------- #
_RENAMED_MODULES = {'utils.collections': 'detectron.utils.collections', }
...
def _raise_key_rename_error(full_key):
    ...
    if isinstance(new_key, tuple):
        ...
        msg = ''
    raise KeyError('Key {} was renamed to {}; please update your config.{}'.
