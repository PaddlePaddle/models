...
class TransAddDelta(object):
    ...
    def perform_trans(self, sample):
        ...
        head_filled = 5
        tail_filled = 5
        mat = numpy.zeros((feature.shape[0] + head_filled + tail_filled, d_frame_dim), dtype="float32")
        #copy first frame
        for i in xrange(head_filled):
            numpy.copyto(
                    mat[i, 0 : frame_dim],  feature[0, :])
        
        numpy.copyto(mat[head_filled : head_filled + feature.shape[0], 0 : frame_dim], feature[:, :])
        
        # copy last frame
        for i in xrange(head_filled + feature.shape[0], mat.shape[0], 1):
            numpy.copyto(
                    mat[i, 0 : frame_dim], feature[feature.shape[0] - 1, :])
    
        nframe = feature.shape[0]
        start = head_filled
        tmp_shape = mat.shape
        mat = mat.reshape((tmp_shape[0] * tmp_shape[1]))
        self._regress(mat, start * d_frame_dim, 
                mat, start * d_frame_dim + frame_dim, 
                frame_dim, nframe, d_frame_dim)
        self._regress(mat, start * d_frame_dim + frame_dim,
                mat, start * d_frame_dim + 2 * frame_dim, 
                frame_dim, nframe, d_frame_dim)
        mat.shape = tmp_shape
