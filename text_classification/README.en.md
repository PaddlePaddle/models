The minimum PaddlePaddle version needed for the code sample in this directory is v0.11.0. If you are on a version of PaddlePaddle order than v0.11.0, [please update your installation](http://www.paddlepaddle.org/docs/develop/documentation/zh/build_and_install/pip_install_cn.html).
...
```text
.
├── images              # Images in this document
│   ├── cnn_net.png
│   └── dnn_net.png
├── infer.py            # Script for prediction
├── network_conf.py     # The various network structures involved in this example are defined in this file, and if you want to further modify the model structure, use this file
├── reader.py           # The interface used to read data, if you customized data format, use this file
├── README.md           # The current document you are reading
├── run.sh              # Running script for training task, if you run the script directly, it will start the training task with the default parameters
...
2. However, too complex models can easily memorize every sample in training samples, but it has no recognition ability for unknown samples that do not appear in training set. This is the **overfitting**.

"No Free Lunch (NFL)" is one of the basic principles of a machine learning task: No model is superior to others inherently. The design and selection of the model is based on understanding the characteristics of different models, but it is also a process of multiple experimental evaluation. In this case, we continue to introduce some of the most commonly used text classification models. Their ability and complexity are different, which can help you compare and learning differences between these models, and choose different models in different scenarios.
...
1. `fc_net`： DNN model，which is a non-sequential model and uses a basic fully connected structure.
2. `convolution_net`：Shallow CNN model，which is a basic sequential model that can handle the variable long sequential input and extract the features within a local region.

We take the task of emotional classification as an example to explain the difference between the sequential model and the non sequential model. Emotional classification is a common text classification task, and the model automatically determines whether the emotion is positive or negative. For example, "not bad" in sentence "The apple is not bad" is the key to determining the emotion of the sentence.

- For the DNN model, we only know that there is a "not" and a "bad" in the sentence. The order relation between them is lost in the input network, and the network no longer has the chance to learn the sequential information between sequences.
- The CNN model accepts text sequences as input and preserves the sequential information between "not bad".

The pros and cons of the two models are summarized as follows:
...
3. In most cases that require some semantic understanding of text classification tasks, the sequential models represented by CNN/RNN are often better than DNN models, such as eliminating ambiguity in text by using context.
...
- **Max-pooling layer**：The max-pooling is carried out on the time series, and eliminates the difference in the number of words in different corpus samples, and extracts the maximum value of each position, e.g., position 0, 1, 2, ..., in the word vector. After being pooled, the vector sequence of the word vector layer is transformed into a vector of a fixed dimension. For example, it is assumed that the vector before the max-pooling is `[[2,3,5],[7,3,6],[1,4,0]]`，Then the max-pooling result is `[7,4,6]`。

- **Fully connected layer**：After the max-pooling, the vector is sent into two continuous hidden layers, and the hidden layers are fully connected.
...
- **Word vector layer**：The word vector layer in CNN is the same as in DNN. It transforms words into fixed dimension vectors, and uses the distance between vectors to express the semantic similarity between words. As shown in Figure 2, the word vector is defined as a linear vector, and then a matrix is formed by concatenating all the word vectors of all the word vectors in the sentence. If the vector dimension of the word is 5, the sentence "The cat sat on the read mat
...
```
The log is output once every 100 batches, and the output information includes: (1) the Pass sequence number; (2) the Batch sequence number; (3) The results of the current Batch evaluation index. The evaluation index is specified when configuring the network topology, and in the above output, the AUC of the training sample set and the error rate index are output.
...
## Using customized data training and prediction
...
2. Writing the data reading interface
    ...
    Next, we only need to pass the data read function `train_reader` as a parameter to the `paddle.batch` interface in the `train.py` script, and then we can read data using the customized data interface. The way to invoke is as follows:
...
3. Modifying the command line parameters
    ...
    - The execution `python train.py --help` can get a detailed description of the startup parameters of the `train.py` script. The main parameters are as follows:
        ...
        - `train_data_dir`：Specify the folder where the training data is located, and you must specify this parameter by using customized data training. Otherwise, `paddle.dataset.imdb` training is used, and `test_data_dir`, `word_dict`, and `label_dict` parameters are ignored.
