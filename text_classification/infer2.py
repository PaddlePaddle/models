...
def infer(topology, data_dir, model_path, word_dict_path, label_dict_path,
    ...
    if use_default_data:
        ...
        word_dict_full = load_dict(word_dict_path)
        word_dict = dict((value, key)
                         for key, value in word_dict.iteritems()[:dict_size])
    ...
    embed_dim = 28  # see network_conf.py
    small = np.random.rand(dict_size, embed_dim)
    parameters = paddle.parameters.Parameters.from_tar(
        gzip.open(model_path, "r"), embedding=["___embedding_0__.w0", small])
    # parameters = paddle.parameters.Parameters.from_tar(
    #     gzip.open(model_path, "r"))
    _table = parameters.get("___embedding_0__.w0")
    print _table.shape
    # small = _table[:5]
