...
def list_modules(**kwargs):
    ...
    for cat, modules in module_by_category.items():
        ...
        for mod in modules:
            print(color_tty.green(mod.name.ljust(max_len)), mod.doc.split('\n')[0])
        print("")
...
def help_module(**kwargs):
    ...
    opts = "\n".join(["{} {}".format(color_tty.green(k.ljust(max_len)), v)
    ...
    for arg in schema.schema.values():
        print("--opt {}.{}={}".format(schema.name, arg.name,
                                      dump_value(arg.default) if arg.has_default() else "<value>"))
...
def analyze_config(**kwargs):
    ...
    for key in config.keys():
        ...
        if key not in modules and not hasattr(config[key], '__dict__'):
            ...
            if type_name in modules:
                ...
                module.update({k: v for k, v in config[key].__dict__.items() if
                               k in module.schema})
            ...
            if isinstance(module[dep], str) and module[dep] != '<value>':
                if module[dep] not in modules:  # not a valid module
                    ...
                    if not dep_mod and dep_mod.mandatory():
                        dep_missing.append(dep)
        override = list(set(module.keys()) - set(default)
                        - set(extra) - set(dep_missing))
        replacement = {}
    ...
    buffer = (re.sub(r"<module config missing>", r"[31m<module config missing>[0m", buffer))
...
if __name__ == '__main__':
    ...
    help_parser = subparsers.add_parser("help", help="show detail options for module")
    generate_parser = subparsers.add_parser("generate", help="generate configuration template")
    analyze_parser = subparsers.add_parser("analyze", help="analyze configuration file")
    ...
    list_group.add_argument("-c", "--category", type=str, default=None,
                            help="list modules for <category>")

    help_parser.add_argument("module", help="module to show info for",
                             choices=list(get_registered_modules().keys()))

    generate_parser.add_argument("modules", nargs='+',
                                 help="include these module in generated configuration template",
                                 choices=list(get_registered_modules().keys()))
    generate_group = generate_parser.add_mutually_exclusive_group()
    generate_group.add_argument("--minimal", action='store_true',
                                help="only include required options")
    generate_group.add_argument("--full", action='store_false', dest='minimal',
