...
- 要使用paddle-lite部署首先需要获取inference model，在tools文件夹下提供了输出inference model的脚本文件export_model.py，运行如下命令即可获取inference model。
```
...
- 在android环境下进行shell 部署
(1)准备模型转换工具生成可用于paddle-lite的部署模型，模型转换工具[opt_linux](https://github.com/PaddlePaddle/Paddle-Lite/releases/download/v2.10/opt_linux)、[opt_mac](https://github.com/PaddlePaddle/Paddle-Lite/releases/download/v2.10/opt_mac)。或者参考[文档](https://paddle-lite.readthedocs.io/zh/develop/user_guides/model_optimize_tool.html)编译您的模型转换工具。使用如下命令转换可以转换inference model到paddle lite的nb模型：
...
```
运行之后即可在当前文件夹下发现mobilenet_v3_small.nb文件。

(2)shell 下进行推理部署,以arm v8 、android系统为例进行部署。
- 准备编译环境
环境要求
* gcc、g++（推荐版本为 8.2.0）
...
adb shell "export LD_LIBRARY_PATH=/data/local/tmp/arm_cpu/; \
           ...
           /data/local/tmp/arm_cpu/mobilenetv3_fp32.nb \
           1,3,224,224 \
           10 10 0 1 1 0" 
...
- lite提供的mobilenet_v1_api demo 的源码分析以及添加前处理方法：
```c++
#include <iostream>
// 引入 C++ API
