...
本文档主要关注Linux GPU/CPU 下模型的混合精度训练推理全流程功能测试，具体测试点如下：
...
* `test_train_inference_python.sh`: 测试Linux上训练、模型动转静、推理功能的脚本，会对`train_amp_infer_python.txt`进行解析，得到具体的执行命令。**该脚本无需修改**。
...
完整的`train_amp_infer_python.txt`配置文件共有32行，包含5个方面的内容。
...
具体内容见[train_amp_infer_python.txt](../../mobilenetv3_prod/Step6/test_tipc/configs/mobilenet_v3_small/train_amp_infer_python.txt)。
...
以训练命令`python3.7 train.py --amp_level=O1 --device=gpu --epochs=2 --data-path=./lite_data`为例，总共包含4个超参数。
...
* 学习率：`--amp_level=O1`，由于配置文件中不包含该项配置，因此可以将其和`train.py`字段（配置文件第14行）放在一起，具体会在2.2.2节详细说明。
...
* 配置文件的第13行直接写`amp_train`即可。
* 第14行配置`amp_train`的具体运行脚本/入口，即上述命令中的`train.py`因此配置文件的14行内容初步可以修改为`amp_train:train.py --amp_level=O1`，考虑到`--amp_level=O1`超参数无法在配置文件中配置，因此可以在这里添加，修改后内容为`amp_train:train.py --amp_level=O1`
...
```bash
# 模型训练
python3.7 train.py --device=gpu --epochs=2 --data-path=./lite_data --amp_level=O1
# 模型动转静
...
2. 环境：安装好PaddlePaddle即可进行混合精度训练推理测试开发
...
在repo的`test_tipc/`目录中新建`configs/model_name`，将文件 [train_amp_infer_python.txt](../../mobilenetv3_prod/Step6/test_tipc/configs/mobilenet_v3_small/train_amp_infer_python.txt) 拷贝到该目录中，其中`model_name`需要修改为您自己的模型名称。

**【实战】**

配置文件的含义解析可以参考 [2.2节配置文件解析](#2.2) 部分。

mobilenet_v3_small的测试开发配置文件可以参考：[train_amp_infer_python.txt](../../mobilenetv3_prod/Step6/test_tipc/configs/mobilenet_v3_small/train_amp_infer_python.txt)。
...
2. Linux GPU/CPU 混合精度训练推理功能测试说明文档：test_tipc/docs/test_amp_train_infer_python.md
...
2. Linux GPU/CPU 混合精度训练推理测试说明文档：[test_train_amp_inference_python.md](../../mobilenetv3_prod/Step6/test_tipc/docs/test_train_amp_inference_python.md)
