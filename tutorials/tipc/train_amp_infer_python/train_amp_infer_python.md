...
通常情况下，O2相比O1训练速度会更快。如果您想获得更好的训练加速效果，我们推荐使用O2模式。下面以O2模式为例，详细介绍如何使用飞桨框架实现混合精度训练。
...
在飞桨框架中，使用AMP-O2模式训练，需要在FP32代码的基础上改动四处：
...
2）在执行训练前，通过`decorate`进行模型参数类型转换：
...
3）在训练循环中，通过`auto_cast`创建AMP-O2训练的上下文环境：
...
- 通常，只需要将模型的前向计算代码`output = model(image)`和loss计算代码`loss = criterion(output, target)`置于`auto_cast`作用域中；
