...
class ESC50(AudioClassificationDataset):
    ...
    def __init__(self,
        ...
        self.bc = mode=='train' and bc_learning
        self.preprocess_funcs = self._preprocess_setup()

    def _read_file(self, input_file: str, split=1) -> List[InputExample]:
        ...
        if self.data_type == 'npz':
            ...
            for fold_tag, data in dataset.items():
                ...
                for sound, label in zip(sounds, labels):
                    example = InputExample(guid=audio_id, source=sound, label=label)
                    audio_id += 1
            ...
            with io.open(input_file, "r", encoding="UTF-8") as f:
                ...
                audio_id = 0
                for line in reader:
                    example = InputExample(guid=audio_id, source=line[0], label=line[1])  # wavfile \t label
                    audio_id += 1
                    examples.append(example)
        else:
            raise NotImplementedError(f'Only soppurts npz|wav_scp data type, but got {self.data_type}')
    ...
    def __getitem__(self, idx):
        if self.bc:  # bc learning, mix audio
            ...
            if self.mode == 'train':
                ...
                sound, target = record['feat'], record['label']
                sound = self._preprocess(sound)[idx%self.n_crops].astype(np.float32)
                # label = np.eye(self.n_class)[int(target)].astype(np.float32)
                label = int(target)
