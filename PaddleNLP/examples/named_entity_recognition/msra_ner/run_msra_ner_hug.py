...
from paddlenlp.datasets import MSRA_NER, MapDatasetWrapper
...
def convert_example(example,
    ...
    if labels_raw:
        ...
        labels_id = [no_entity_id] + labels_trun + [no_entity_id]
    if not is_test:
...
def do_train(args):
    ...
    train_dataset = MapDatasetWrapper(msra_dataset['train'])
    dev_dataset = MapDatasetWrapper(msra_dataset['test'])
    tokenizer = BertTokenizer.from_pretrained(args.model_name_or_path)

    label_list = get_label_list(train_dataset['ner_tags'])
    ...
    metric = ChunkEvaluator(
        label_list=msra_dataset['train'].features['ner_tags'].feature.names)
