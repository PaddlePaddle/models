...
class Lambada_Eval_Dataset(paddle.io.Dataset):
    def __init__(self, tokens, seq_len, pad_idx):
        self.seq_len = seq_len
        self.pad_idx = pad_idx
        self.tokens = tokens
    ...
    if not args.cloze_eval:
        ...
        with open(args.eval_path, 'r') as f:
            for line in f.readlines():
                ...
                tokenized_data.append(tokenizer.encode(text))
        val_dataset = Lambada_Eval_Dataset(tokenized_data, seq_len, pad_token)
    ...
    with paddle.no_grad():
        for step, batch in enumerate(eval_data_loader):
            ...
            if not args.cloze_eval:
                ...
                loss = paddle.sum(masked_lm_loss.reshape([-1]) * loss_mask)
                total_score += loss.numpy() / (args.num_tokenized_tokens - 1)
            else:
                outputs = paddle.argmax(preds, -1).reshape([-1])
                acc = paddle.cast((outputs == labels.reshape([-1])), 'float32')
                acc = paddle.sum(acc * loss_mask)
