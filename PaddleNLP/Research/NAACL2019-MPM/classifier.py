    ...
    if args.sub_model_type == 'raw':
        ...
        for h in [3, 4, 5]:
            conv_feats = fluid.layers.sequence_conv(
                input=bert_seq_out,
                num_filters=cnn_hidden_size,
                filter_size=h)
            conv_feats = fluid.layers.batch_norm(
                input=conv_feats,
                act="relu")
        ...
        cls_feats = fluid.layers.sequence_pool(input=gru_output, pool_type='max')

    elif args.sub_model_type == 'ffa':
        bert_seq_out = bert.get_sequence_output()
        attn = fluid.layers.fc(
            input=bert_seq_out,
            num_flatten_dims=2,
            size=1,
            act='tanh')
        ...
        raise NotImplementedError("%s is not implemented!" % args.sub_model_type)
