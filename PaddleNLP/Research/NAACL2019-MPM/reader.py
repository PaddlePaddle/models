...
class DataProcessor(object):
    ...
    def data_generator(self,
        ...
        """
        if phase == 'train':
            examples = self.get_train_examples(self.data_dir, drop_keyword=drop_keyword)
            self.num_examples['train'] = len(examples)
        elif phase == 'dev':
...
class SemevalTask9Processor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, header=False, drop_keyword=False):
        ...
        examples = []
        if drop_keyword:
            keywords = [line.strip() for line in open(data_dir + '/../keywords')]
    ...
    def get_test_examples(self, data_dir, header=False):
        lines = self._read_csv(data_dir + '/SubtaskA_EvaluationData_labeled.csv')
        examples = []
        for i, line in enumerate(lines):
            if i == 0 and header:
                continue
