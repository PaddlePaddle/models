...
class GRUEncoder(nn.Layer):
    ...
    The output is of shape `(batch_size, hidden_size*2)` if GRU is bidirect;
    ...
    And if direction is bidirect, the we concat the hidden state of the last forward 
    ...
    Args:
        ...
        direction (obj:`str`, optional, defaults to obj:`forwrd`): The direction of the network. 
            It can be "forward" and "bidirect".
    ...
    def forward(self, inputs, sequence_length):
        ...
        if not self._pooling_type:
            ...
            # If gru is not bidirect, then output is the hidden state of the last time step 
            # at last layer. Output is shape of `(batch_size, hidden_size)`.
            # If gru is bidirect, then output is concatenation of the forward and backward hidden state 
            ...
            # If gru is not bidirect, output is shape of `(batch_size, hidden_size)`.
            # If gru is bidirect, then output is shape of `(batch_size, hidden_size*2)`.
...
class LSTMEncoder(nn.Layer):
    ...
    The input to this module is of shape `(batch_size, num_tokens, input_size)`, 
    The output is of shape `(batch_size, hidden_size*2)` if LSTM is bidirect;
    If not, output is of shape `(batch_size, hidden_size)`.
    ...
    def forward(self, inputs, sequence_length):
        ...
        if not self._pooling_type:
            # We exploit the `last_hidden` (the hidden state at the last time step for every layer)
            # to create a single vector.
            # If lstm is not bidirect, then output is the hidden state of the last time step 
            # at last layer. Output is shape of `(batch_size, hidden_size)`.
            # If lstm is bidirect, then output is concatenation of the forward and backward hidden state 
            # of the last time step at last layer. Output is shape of `(batch_size, hidden_size*2)`.
            if self._direction != 'bidirect':
                output = last_hidden[-1, :, :]
            else:
            ...
            # We exploit the `encoded_text` (the hidden state at the every time step for last layer)
            # to create a single vector. We perform pooling on the encoded text.
            # If lstm is not bidirect, output is shape of `(batch_size, hidden_size)`.
            # If lstm is bidirect, then output is shape of `(batch_size, hidden_size*2)`.
            if self._pooling_type == 'sum':
                output = paddle.sum(encoded_text, axis=1)
            elif self._pooling_type == 'max':
                output = paddle.max(encoded_text, axis=1)
...
class RNNEncoder(nn.Layer):
    ...
    The input to this module is of shape `(batch_size, num_tokens, input_size)`, 
    The output is of shape `(batch_size, hidden_size*2)` if RNN is bidirect;
    If not, output is of shape `(batch_size, hidden_size)`.
    ...
    def forward(self, inputs, sequence_length):
        ...
        if not self._pooling_type:
            # We exploit the `last_hidden` (the hidden state at the last time step for every layer)
            # to create a single vector.
            # If rnn is not bidirect, then output is the hidden state of the last time step 
            # at last layer. Output is shape of `(batch_size, hidden_size)`.
            # If rnn is bidirect, then output is concatenation of the forward and backward hidden state 
            # of the last time step at last layer. Output is shape of `(batch_size, hidden_size*2)`.
            if self._direction != 'bidirect':
                output = last_hidden[-1, :, :]
            else:
            ...
            # We exploit the `encoded_text` (the hidden state at the every time step for last layer)
            # to create a single vector. We perform pooling on the encoded text.
            # If rnn is not bidirect, output is shape of `(batch_size, hidden_size)`.
            # If rnn is bidirect, then output is shape of `(batch_size, hidden_size*2)`.
            if self._pooling_type == 'sum':
                output = paddle.sum(encoded_text, axis=1)
            elif self._pooling_type == 'max':
                output = paddle.max(encoded_text, axis=1)
