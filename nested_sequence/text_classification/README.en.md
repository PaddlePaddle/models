The minimum PaddlePaddle version needed for the code sample in this directory is v0.11.0. If you are on a version of PaddlePaddle earlier than v0.10.0, [please update your installation](http://www.paddlepaddle.org/docs/develop/documentation/en/build_and_install/pip_install_en.html).
---
...
PaddlePaddle implementation of the network structure of the code see `network_conf.py`.
...
- **Linear projection layer**: the results obtained from different convolutions are spliced into a long vector after the maximum pool level. Then, a linear projection is used to get the representation vectors of corresponding single layer sequences.
...
After getting the expression vectors of each sentence, all the sentence vectors are passed through an average pool level, and a vector representation of a sample is obtained. The vector output the final prediction result through a fully connected layer. The code is as follows:
```python
...
Modify the `config.py` to adjust the parameters. For example, by modifying the `use_gpu` parameter to specify whether or not to use GPU for training.
...
The input data format is as follows: one sample of each behavior is separated by `\t` , the first column is a class label, and the second column is the content of the input text. The following are two sample data:
