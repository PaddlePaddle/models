...
def show_parameter_init_info(parameters):
    ...
    for p in parameters:
        ...
        logger.info(("%-25s : initial_mean=%-7.4f initial_std=%-7.4f "
                    ...
                    (p,
                     parameters.__param_conf__[p].initial_mean,
                     parameters.__param_conf__[p].initial_std,
                     p_val.mean(),
                     p_val.std(),
                     parameters.__param_conf__[p].dims))
    logger.info("\n")
...
def show_parameter_status(parameters):
    ...
    for p in parameters:
        ...
        logger.info(
            ...
            (p,
             abs_val.mean(), abs_val.max(),
             abs_grad.mean(), abs_grad.max(),
...
def train(conf):
    ...
    # create optimizer
    rmsprop_optimizer = pd.optimizer.RMSProp(
        learning_rate=conf.learning_rate,
        rho=conf.rho,
        epsilon=conf.epsilon,
        model_average=pd.optimizer.ModelAverage(
            average_window=conf.average_window,
            max_average_window=conf.max_average_window))

    # create trainer
    trainer = pd.trainer.SGD(
        cost=cost,
