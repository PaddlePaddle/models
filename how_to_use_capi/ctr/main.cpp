...
int main(int argc, char** argv) {
  ...
  const char* config_bin_path = argv[1];
  const char* model_path = argv[2];
  const char* infer_data_path = argv[3];

  // Reading config binary file. It is generated by `convert_protobin.sh`
  long size = 0;
  void* config_buf = read_config(config_bin_path, &size);
  if (config_buf == NULL) return 0;
  ...
  if ((file.rdstate() & ifstream::failbit) != 0) {
    cerr << "Open " << infer_data_path << " error" << endl;
    return 0;
  }
  ...
  // Initalize Paddle
  char* args[] = {"--use_gpu=False"};
  CHECK(paddle_init(1, (char**)args));
