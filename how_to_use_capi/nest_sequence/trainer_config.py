...
def lstm_group(lstm_group_input):
    group_input = paddle.layer.mixed(size=hidden_size*4,
...
lstm_nest_group = paddle.layer.recurrent_group(input=paddle.layer.SubsequenceInput(emb),
                                         step=lstm_group, name="lstm_nest_group")

lstm_last = paddle.layer.last_seq(input=lstm_nest_group, agg_level=paddle.layer.AggregateLevel.TO_SEQUENCE)

lstm_average = paddle.layer.pooling(input=lstm_last,pooling_type=paddle.pooling.Avg(),
                                    agg_level=paddle.layer.AggregateLevel.TO_NO_SEQUENCE)

output = paddle.layer.mixed(size=label_dim,
