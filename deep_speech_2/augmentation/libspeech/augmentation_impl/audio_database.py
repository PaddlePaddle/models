...
def stream_audio_index(fname, UNK=UNK_TAG):
    ...
    Args:
        fname (str): audio index path
        UNK (str): UNK token to denote that certain audios are not tagged.
...
def print_audio_index(idx, duration, size, relpath, tags, file=sys.stdout):
    ...
    Args:
        idx (int): Audio file id.
        duration (float): length of the audio in seconds
        size (int): size of the file in bytes
        relpath (str): relative path w.r.t. to the root noise directory.
        tags (list(str)): list of tags
        file (file): file to which we want to write an audio record.
    """
    file.write("{}\t{:.6f}\t{}\t{}"
...
class AudioIndex(object):
    ...
    def has_audio(self, distr=None):
        """
        Args:
            distr (dict): The target distribution of audio tags that we want to
                match. If this is not supplied, the function simply checks that
                there are some audio files.

        Returns:
            True if there are audio files.
        """
        if distr is None:
    ...
    def _load_all_records_from_disk(self, audio_dir, idx_fname, bin_size):
        ...
        Returns:
            whether the records were read from the disk. The assumption is
    ...
    def _build_index_from_records(self, tag_list):
        ...
        Args:
            tag_list (list): List of tags we are interested in sampling from.
        ...
        Returns:
            whether the index was built from scratch
        """
    ...
    def refresh_records_from_index_file(self,
        ...
        Args:
            ...
            idx_fname (str): audio index file name
            tag_list (list): list of tags we are interested in loading;
                if empty, we load all.
            bin_size (float): optional argument for controlling the granularity
                of duration bins.
    ...
    def sample_audio(self, duration, rng=None, distr=None):
        ...
        Args:
            duration (float): minimum desired audio duration
            rng (random.Random): random number generator
            distr (dict): target distribution of audio tags. If not provided,
                all audio files are sampled uniformly at random.

        Returns:
            success, (duration, file_size, path)
        """
        if duration < 0.0:
