...
class FPN(object):
    ...
    def _add_topdown_lateral(self, body_name, body_input, upper_output):
        ...
        fan_in = body_input.shape[1] 
        lateral = fluid.layers.conv2d(
            body_input,
            self.num_chan,
            1,
            param_attr=ParamAttr(
                name=lateral_name + "_w", initializer=Xavier(fan_out=fan_in)),
            bias_attr=ParamAttr(
    ...
    def get_output(self, body_dict):
        ...
        self.fpn_inner_output[0] = fluid.layers.conv2d(
            ...
            1,
            param_attr=ParamAttr(
                name=fpn_inner_name + "_w", initializer=Xavier(fan_out=fan_in)),
            bias_attr=ParamAttr(
        ...
        for i in range(num_backbone_stages):
            ...
            fan_in = self.fpn_inner_output[i].shape[1] * 3 * 3
            fpn_output = fluid.layers.conv2d(
                ...
                padding=1,
                param_attr=ParamAttr(
                    name=fpn_name + "_w", initializer=Xavier(fan_out=fan_in)),
                bias_attr=ParamAttr(
                    name=fpn_name + "_b",
                    learning_rate=2.,
                    regularizer=L2Decay(0.)),
        ...
        if self.has_extra_convs and self.max_level > highest_backbone_level:
            ...
            for i in range(highest_backbone_level + 1, self.max_level + 1):
                ...
                fan_in = fpn_blob_in.shape[1] * 3 * 3
