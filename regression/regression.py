...
def regression_net(input1_dict_dim, input2_dict_dim, is_generating=False):
    ...
    #input2 architecture
    input2 = paddle.layer.data(name='input2_word',type=paddle.data_type.integer_value_sequence(input2_dict_dim))
    ...
    input2_vec = paddle.layer.pooling(
        ...
        bias_attr=paddle.attr.ParameterAttribute(name='_avg.bias_basic', is_static=True))
    ...
    hidden_input2 = paddle.layer.fc(
        ...
        bias_attr=paddle.attr.ParameterAttribute(name='_hidden_input2.bias', is_static=True))


    #input1 architecture
    input1 = paddle.layer.data(name='input1_word',type=paddle.data_type.integer_value_sequence(input1_dict_dim))
    ...
    input1_vec = paddle.layer.pooling(
        ...
        pooling_type=paddle.pooling.Sum(),
        #act=paddle.activation.Tanh(),
        bias_attr=paddle.attr.ParameterAttribute(name='_avg.bias_input1', initial_std=0.01))

    hidden_input1 = paddle.layer.fc(
        input=input1_vec,
        size=hidden_dim,
        act=paddle.activation.Tanh(),
        param_attr=paddle.attr.ParameterAttribute(name='_hidden_input1.w', initial_std=0.03),
