    ...
    def build_input(self):
        ...
        if self.mode == 'train':
            self.gt_box = fluid.data(
                name='gt_box', shape=[-1, 5], dtype='float32', lod_level=1)
        else:
            self.gt_box = fluid.data(
                name='gt_box', shape=[-1, 8], dtype='float32', lod_level=1)
        self.gt_label = fluid.data(
            name='gt_class', shape=[-1, 1], dtype='int32', lod_level=1)
        self.is_crowd = fluid.data(
            name='is_crowed', shape=[-1, 1], dtype='int32', lod_level=1)
        self.im_info = fluid.data(
            name='im_info', shape=[-1, 3], dtype='float32')
        self.im_id = fluid.data(name='im_id', shape=[-1, 1], dtype='int64')
        self.difficult = fluid.data(
            name='is_difficult', shape=[-1, -1], dtype='float32', lod_level=1)
        if self.mode == 'train':
            feed_data = [
