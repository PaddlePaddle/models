...
# 该文件除了剔除了dropout层，其余内容和danet.py一样
import shutil
...
class BottleneckBlock(fluid.dygraph.Layer):
    ...
    def forward(self, inputs):
        x = self._conv0(inputs)  # conv1x1
        x = self._conv1(x)  # conv3x3
        x = self._conv2(x)  # conv1x1
        # print(x.shape)
        if self.same:
            skip = inputs
        else:
...
class CAM(fluid.dygraph.Layer):
    def __init__(self,
        ...
        self.gamma = fluid.layers.create_parameter(shape=[1],
                                                   dtype='float32',
                                                   is_bias=True,
                                                   attr=fluid.ParamAttr(learning_rate=1.0,
                                                                        name='cam_gamma'),
...
class DAHead(fluid.dygraph.Layer):
    ...
    def forward(self, inputs):
        ...
        position_out = self._pam_last_conv(position)
        channel_out = self._cam_last_conv(channel)

        feat_sum = fluid.layers.elementwise_add(position, channel, axis=1)
        feat_sum_out = self._last_conv(feat_sum)
