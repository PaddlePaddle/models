...
def main():
    ...
    train_pyreader, train_feed_vars = create_feed(train_feed, use_pyreader=True)
    ...
    with fluid.program_guard(eval_prog, startup_prog):
        with fluid.unique_name.guard():
            model = create(main_arch)
            _, test_feed_vars = create_feed(eval_feed, use_pyreader=False)
    ...
    def eval_func(program, scope):
        ...
        if len(best_box_ap_list) == 0:
            ...
            checkpoint.save(exe,
                            ...
                            os.path.join("output", "best_model"))
