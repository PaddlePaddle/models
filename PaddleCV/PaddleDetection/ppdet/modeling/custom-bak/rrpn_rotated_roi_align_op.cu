...
//         const T* offset_bottom_rois = bottom_rois + n * 5;
//         int roi_batch_ind = roi_batch_id_data[n];
//         T* offset_bottom_diff = bottom_diff + (roi_batch_ind * channels + c)
//         * height * width;
//
//         //int bottom_index = argmax_data[index];
//
...
//         // Atomic add
//
//         if (min_y > 0 && min_x  > 0 && min_y < height - 1 && min_x < width -
//         1)
//             platform::CudaAtomicAdd(offset_bottom_diff + min_y * width +
//             min_x, static_cast<T>(v1));
//         if (min_y > 0 && max_x < width - 1 && min_y < height - 1 && max_x >
//         0)
//             platform::CudaAtomicAdd(offset_bottom_diff + min_y * width +
//             max_x, static_cast<T>(v2));
//         if (max_y < height - 1 && max_x < width - 1 && max_y > 0 && max_x >
//         0)
//             platform::CudaAtomicAdd(offset_bottom_diff + max_y * width +
//             max_x, static_cast<T>(v3));
//         if (max_y < height - 1 && min_x > 0 && max_y > 0 && min_x < width -
//         1)
//             platform::CudaAtomicAdd(offset_bottom_diff + max_y * width +
//             min_x, static_cast<T>(v4));
//
//         //}
//
//   }
