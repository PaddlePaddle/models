...
//    const T* target_box_data, const int row, const int len,
...
//    T prior_box_width = prior_box_data[row_idx * len + 2];
//    T prior_box_height = prior_box_data[row_idx * len + 3];
//    T prior_box_center_x = prior_box_data[row_idx * len];
//    T prior_box_center_y = prior_box_data[row_idx * len + 1];
//    T prior_box_angle = prior_box_data[row_idx * len + 4];
//
//    T target_box_center_x = target_box_data[row_idx * len];
//    T target_box_center_y = target_box_data[row_idx * len + 1];
//    T target_box_width = target_box_data[row_idx * len + 2];
//    T target_box_height = target_box_data[row_idx * len + 3];
//    T target_box_angle = target_box_data[row_idx * len + 4];
//
//    output[idx * len] =
...
//    output[idx * len + 4] = target_box_angle - prior_box_angle;
...
//      output[idx * len + 4] *= prior_box_var_data[prior_var_offset + 4];
//    } else if (var_size == 5) {
//      for (int k = 0; k < 5; ++k) {
...
//    }
//
//    if ((target_box_angle <= -30 && prior_box_angle >= 120) ||
//    (target_box_angle > -30 && prior_box_angle < 120))
//        {
//          output[idx * len + 4] = output[idx * len + 4] + 180.0;
//        }
//        else
//        {
//          output[idx * len + 4] = output[idx * len + 4] - 180.0;
//        }
//        if ((target_box_angle >= 120 && prior_box_angle <= -30) ||
//        (target_box_angle < 120 && prior_box_angle > -30))
//        {
//          output[idx * len + 4] = output[idx * len + 4] - 180.0;
//        }
//        else
...
//          output[idx * len + 4] = output[idx * len + 4] + 180.0;
//        }
//    output[idx * len + 4] = (3.14159265358979323846264338327950288 / 180) *
//    output[idx * len + 4];
//  }
//}
enum class BoxCodeType { kDecodeCenterSize = 1 };

inline BoxCodeType GetBoxCodeType(const std::string& type) {
  if (type == "decode_center_size") {
...
__global__ void DecodeCenterSizeKernel(const T* prior_box_data,
  ...
  if (idx < row) {
    ...
    // output[idx * len] = target_box_center_x;
    // output[idx * len + 1] = target_box_center_y;
    // output[idx * len + 2] = target_box_width;
    // output[idx * len + 3] = target_box_height;
    // output[idx * len + 4] = target_box_angle;
...
public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    //      prior_box_data, prior_box_var_data, target_box_data, row, len,
