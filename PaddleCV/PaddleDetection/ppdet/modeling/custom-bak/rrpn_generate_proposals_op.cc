/*opyright (c) 2018 PaddlePaddle Authors. All Rights Reserved.
...
static inline void RFilterBoxes(const platform::DeviceContext &ctx,
  ...
  for (int i = 0; i < boxes->dims()[0]; ++i) {
    ...
    // T ws_origin_scale =
    //    (boxes_data[5 * i + 2]) / im_scale + 1;
    // T hs_origin_scale =
    //    (boxes_data[5 * i + 3]) / im_scale + 1;
    // T x_ctr = boxes_data[5 * i];
    // T y_ctr = boxes_data[5 * i + 1];
...
public:
  ...
  std::pair<Tensor, Tensor> ProposalForOneImage(
    ...
    /*
    std::ofstream file;
    file.open("/paddle/sort.txt");
    file.precision(11);
    for (int k = 0;  k < pre_nms_top_n; ++k) {
       file << index[k] << " " << scores_data[int(index[k])] <<std::endl;
    }
    file.close();
    */
    // for(int k=0; k < pre_nms_top_n; k++)
    ...
    file.precision(11);
    for (int k = 0; k < pre_nms_top_n; ++k) {
      file << index[k] << " " << scores_[k] << std::endl;
    }
    file.close();
