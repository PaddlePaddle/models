...
class RPNHead(object):
    ...
    def get_proposals(self, body_feats, im_info, mode='train'):
        ...
        if self.num_classes == 1:
            ...
            rpn_cls_prob_slice = fluid.layers.slice(rpn_cls_prob_tmp, axes=[4], starts=[1], ends=[81])
            rpn_cls_prob, _ = fluid.layers.topk(rpn_cls_prob_slice, 1, name='rpn_cls_prob_max')
            rpn_cls_prob = fluid.layers.reshape(rpn_cls_prob, shape=(0, 0, 0, -1))
            #rpn_cls_prob = fluid.layers.reduce_sum(rpn_cls_prob_slice, dim=4)
...
class FPNRPNHead(RPNHead):
    ...
    def _get_single_proposals(self, body_feat, im_info, feat_lvl, mode='train'):
        ...
        if self.num_classes == 1:
            ...
            rpn_cls_score_fpn_prob = fluid.layers.softmax(
                rpn_cls_score_fpn, use_cudnn=False, name='rpn_cls_prob_fpn' + (feat_lvl))
            rpn_cls_prob_fpn = fluid.layers.reduce_max(rpn_cls_prob_fpn, dim=-1)
