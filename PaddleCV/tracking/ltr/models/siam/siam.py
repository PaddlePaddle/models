...
def SiamMask_ResNet50_base(backbone_pretrained=True, scale_loss=None):
    backbone = resnet50(
        'ResNet50', pretrained=backbone_pretrained, output_layers=[0,1,2])
    
    neck = AdjustAllLayer(in_channels=[1024], out_channels=[256], is_test=False)
    
    rpn_head = DepthwiseRPN(anchor_num=5, in_channels=256, out_channels=256, is_test=False)

    mask_head = MaskCorr(in_channels=256, hidden=256, out_channels=3969, is_test=False)
...
def SiamMask_ResNet50_sharp(backbone_pretrained=False, scale_loss=None):
    backbone = resnet50(
        'ResNet50', pretrained=backbone_pretrained, output_layers=[0,1,2])
