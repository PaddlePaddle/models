...
class LocalConv2d(fluid.dygraph.Layer):
    ...
    def forward(self, x):       
        ...
        for i in range(0, self.num_rows):
            tmp = fluid.layers.slice(x, axes=[2], starts=[i*t], ends=[i*t + (t + self.pad *2)])           
...
class RPN(fluid.dygraph.Layer):
    ...
    def forward(self, inputs):
        ...
        if self.phase == "train":
            ...
            feat_size = calc_output_size(np.array(self.conf.crop_size), feat_stride)
...
def build(conf, backbone, phase='train'):
    ...
    if 'pretrained' in conf and conf.pretrained is not None:
        ...
        for layer in conv_layers:
            ...
            src_weights[dst_weight_key] = np.tile(src_weights[src_weight_key], (conf.bins,1,1,1))   
            src_weights[dst_bias_key] = np.tile(src_weights[src_bias_key], conf.bins)
