...
def video_process(video_list,
    ...
    for video_name in video_list:
        df = pd.read_csv("data/output/BMN_results/" + video_name + ".csv")
        if len(df) > 1:
...
def bmn_post_processing(video_dict, subset):
    ...
    if subset == 'validation':
        outfile = open("data/evaluate_results/bmn_results_%s.json" % subset,
                       "w")
    elif subset == 'test':
        outfile = open("data/predict_results/bmn_results_%s.json" % subset, "w")

#    outfile=open("data/evaluate_results/bmn_results_%s.json" %subset,"w")
