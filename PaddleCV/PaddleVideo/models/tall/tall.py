...
class TALL(ModelBase):
	...
	self.visual_feature_dim = self.get_config_from_sec("model", 'visual_feature_dim')
	self.semantic_size = self.get_config_from_sec("model","MODEL.semantic_size")
	...
	self.feats_dimen = self.get_config_from_sec("train", "feats_dimen"）
	...
	offs = self.network_outputs[1]
   	...
   	offset_pred = fluid.layers.concat(input=[p_reg_diag, l_reg_diag], axis=1)
    	...
    	self.loss_ = fluid.layers.mean(loss)
