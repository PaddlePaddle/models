...
class ConvBNReluLayer(fluid.dygraph.Layer):
    def __init__(self, in_channels, out_channels, filter_size, stride=1, groups=1, padding=1, is_test=False):
        ...
        self.conv = nn.Conv2D(num_channels=in_channels,
                              ...
                              groups=groups, bias_attr=bias_init(),
...
class FCBNReluLayer(fluid.dygraph.Layer):
    def __init__(self, in_channels, out_channels, in_size, is_bias=True, is_bn=True, is_relu=True, is_test=False):
        super(FCBNReluLayer, self).__init__()
        self.is_bn = is_bn
        self.is_relu = is_relu

        if is_bias:
            bias_init = fluid.ParamAttr(initializer=fluid.initializer.ConstantInitializer(0.))
        else:
            bias_init = False
        self.linear = nn.Linear(in_channels*in_size*in_size, out_channels, bias_attr=bias_init)
        self.bn = nn.BatchNorm(out_channels,
                               param_attr=norm_weight_init(),
                               bias_attr=norm_bias_init(),
                               act=None, momentum=0.9,
...
class AtomIouNet(fluid.dygraph.Layer):
    def __init__(self, name, input_dim=(128, 256), pred_input_dim=(256, 256), pred_inter_dim=(256, 256), is_test=False):
        super(AtomIouNet, self).__init__(name)
        self.name = self.full_name()
        self.conv3_1r = ConvBNReluLayer(input_dim[0], 128, filter_size=3, stride=1, is_test=is_test)
        self.conv3_1t = ConvBNReluLayer(input_dim[0], 256, filter_size=3, stride=1, is_test=is_test)

        self.conv3_2t = ConvBNReluLayer(256, pred_input_dim[0], filter_size=3, stride=1, is_test=is_test)

        self.fc3_1r = ConvBNReluLayer(128, 256, filter_size=3, stride=1, padding=0, is_test=is_test)

        self.conv4_1r = ConvBNReluLayer(input_dim[1], 256, filter_size=3, stride=1, is_test=is_test)
        self.conv4_1t = ConvBNReluLayer(input_dim[1], 256, filter_size=3, stride=1, is_test=is_test)

        self.conv4_2t = ConvBNReluLayer(256, pred_input_dim[1], filter_size=3, stride=1, is_test=is_test)

        self.fc34_3r = ConvBNReluLayer(512, pred_input_dim[0], filter_size=1, stride=1, padding=0, is_test=is_test)
        ...
        self.fc4_rt = FCBNReluLayer(pred_input_dim[1], pred_inter_dim[1], in_size=3, is_test=is_test)

        bias_init = fluid.initializer.ConstantInitializer(0.)
        self.iou_predictor = nn.Linear(pred_inter_dim[0]+pred_inter_dim[1], 1, bias_attr=bias_init)
    ...
    def predict_iou(self, filter, feat2, proposals):
        ...
        batch_roi_nums = np.array([num_proposals_per_batch] * batch_size).astype(np.int64)
        batch_roi_nums = fluid.dygraph.to_variable(batch_roi_nums)

        # input proposals2 is in format xywh, convert it to x0y0x1y1 format
        proposals_xyxy = fluid.layers.concat([proposals[:, :, 0:2],
        ...
        roi3t = fluid.layers.prroi_pool(c3_t_att, roi2, 1 / 8., 5, 5, batch_roi_nums=batch_roi_nums)
        roi4t = fluid.layers.prroi_pool(c4_t_att, roi2, 1 / 16., 3, 3, batch_roi_nums=batch_roi_nums)
        ...
        iou_pred = fluid.layers.reshape(iou_pred, [batch_size, num_proposals_per_batch])
    ...
    def forward(self, feat1, feat2, bb1, proposals2):
        ...
        feat2 = [fluid.layers.reshape(f, (batch_size * num_test_images, *f.shape[-3:])) for f in feat2]
        ...
        pred_iou = fluid.layers.reshape(pred_iou, [num_test_images, batch_size, -1])
        return pred_iou
    ...
    def get_filter(self, feat1, bb1):
        ...
        roi1 = fluid.layers.concat([bb1[:, 0:2], bb1[:, 0:2] + bb1[:, 2:4]], axis=1)
        roi1.stop_gradient = False

        roi3r = fluid.layers.prroi_pool(c3_r, roi1, 1 / 8., 3, 3, batch_roi_nums)

        c4_r = self.conv4_1r(feat4_r)
        roi4r = fluid.layers.prroi_pool(c4_r, roi1, 1 / 16., 1, 1, batch_roi_nums)
...
def atom_iounet(name, input_dim=(128, 256), pred_input_dim=(256, 256), pred_inter_dim=(256, 256)):
    return AtomIouNet(name, input_dim=input_dim,
