...
def load_network(network_dir=None,
                 checkpoint=None,
    ...
    if checkpoint is None:
        ...
        checkpoint_list = sorted(
            net_path.glob('*_ep{:04d}.pth.tar'.format(checkpoint)))
    ...
    if 'constructor' in checkpoint_dict and checkpoint_dict[
        ...
        for arg, val in kwargs.items():
            if arg in net_constr.kwds.keys():
                ...
                print(
                    'WARNING: Keyword argument "{}" not found when loading network.'.
                    format(arg))
...
def _setup_legacy_env():
    ...
    for m in ('model_constructor', 'stats', 'settings', 'local'):
        ...
        sys.modules['dlframework.common.utils.' + m] = sys.modules['ltr.admin.'
                                                                   + m]
