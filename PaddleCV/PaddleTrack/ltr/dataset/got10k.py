...
class Got10k(BaseDataset):
    ...
    def __init__(self, root=None, filter=None, image_loader=default_image_loader, split=None, seq_ids=None):
        """
        args:
        ...
        if split == 'vot-train':
            ...
            with open(os.path.join(ltr_path, 'data_specs', 'got10k_prohibited_for_VOT.txt')) as f:
                prohibited = [l.strip() for l in f.readlines()]
            print('GOT10K before: {}'.format(len(self.sequence_list)))
            self.sequence_list = [x for x in self.sequence_list if x not in prohibited]
