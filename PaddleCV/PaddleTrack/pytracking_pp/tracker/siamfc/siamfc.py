...
class SiamFC(BaseTracker):
    ...
    def initialize(self, image, state, *args, **kwargs):
        ...
        self.scale_factors = self.params.scale_step ** np.linspace(
            -(self.params.scale_num // 2),self.params.scale_num // 2, self.params.scale_num)
        ...
        exemplar_image = self._crop_and_resize(image, self.center, self.z_sz,
                                               out_size=self.params.exemplar_sz,
                                               pad_color=self.avg_color)
        self.exemplar_img_1s = exemplar_image[np.newaxis, :, :, :]
        self.exemplar_img = np.transpose(self.exemplar_img_1s, [0, 3, 1, 2]).astype(np.float32)
        self.exemplar_img = np.repeat(self.exemplar_img, self.params.scale_num, axis=0)
    ...
    def track(self, image):
        ...
        instance_images = np.transpose(instance_images, [0, 3, 1, 2]).astype(np.float32)
