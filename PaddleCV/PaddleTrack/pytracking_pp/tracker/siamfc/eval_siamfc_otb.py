...
from pytracking_pp.pysot_toolkit.utils import success_overlap, success_error
import json
from pytracking_pp.tracker.siamfc.siamfc import SiamFC

from tqdm import tqdm

from pytracking_pp.parameter.siamfc.default import parameters
...
class ValidOTB(SiamFC):
    def __init__(self, dataset_root, dataset_name, params):
        ...
        if dataset_name not in dataset_list:
            raise Exception("ValidOTB's dataset_name can only be one of {}".format(dataset_list))
    ...
    def inference(self, epoch):
        ...
        # load videos
        pbar = tqdm(self.meta_data.keys(), desc='loading ' + self.dataset_name, ncols=100)
        ...
        precision = success_error(np.array(gt_center), np.array(tracker_center), thresholds, len(gtbb))
        print("####AUC:{}, Precision:{}".format(np.mean(auc), np.mean(precision)))
...
parser.add_argument('--dataset_dir', type=str, default="/paddle/Datasets/OTB100",
                    help="the path of OTB dataset")
parser.add_argument('--dataset_name', type=str, default="CVPR13",
...
if __name__ == '__main__':
    ...
    for i in range(start_epoch, end_epoch, 1):
        ...
        valid = ValidOTB(dataset_root=args.dataset_dir, dataset_name=args.dataset_name, params=params)
        ...
        if auc > best_auc:
            best_auc = auc
            best_epoch = i
        print("####Best AUC is {}, corresponding epoch is {}".format(best_auc, best_epoch))
