...
from pytracking_pp.features import augmentation
from pytracking_pp.libs import dcf, operation, fourier
from pytracking_pp.libs.optimization import ConjugateGradient, GaussNewtonCG, GradientDescentL2
from pytracking_pp.libs.paddle_utils import mod, n2p, \
    leaky_relu, dropout2d
from pytracking_pp.libs.tensorlist import TensorList
from pytracking_pp.tracker.atom.optim import FactorizedConvProblem, ConvProblem
from pytracking_pp.tracker.base.basetracker import BaseTracker
...
class ATOM(BaseTracker):
    ...
    def initialize(self, image, state, *args, **kwargs):
        ...
        self.pos = np.array([state[1] + (state[3] - 1) / 2, state[0] + (state[2] - 1) / 2], 'float32')
        ...
        if getattr(self.params, 'search_area_shape', 'square') == 'square':
            self.img_sample_sz = np.ones((2,), 'float32') * np.round(
                np.sqrt(np.prod(self.base_target_sz * self.params.search_area_scale)))
        elif self.params.search_area_shape == 'initrect':
            self.img_sample_sz = np.round(self.base_target_sz * self.params.search_area_scale)
        else:
            raise ValueError('Unknown search area shape')
        if self.params.feature_size_odd:
            self.img_sample_sz += feat_max_stride - mod(self.img_sample_sz, (2 * feat_max_stride))
        else:
            self.img_sample_sz += feat_max_stride - mod((self.img_sample_sz + feat_max_stride), (2 * feat_max_stride))
        ...
        if getattr(self.params, 'window_output', False):
            if getattr(self.params, 'use_clipped_window', False):
                self.output_window = dcf.hann2d_clipped(self.output_sz.astype('long'),
                                                        self.output_sz.astype(
                                                            'long') * self.params.effective_search_area / self.params.search_area_scale,
                                                        centered=False)
            else:
                self.output_window = dcf.hann2d(self.output_sz.astype('long'), centered=False)
    ...
    def track(self, image):
        ...
        im = image.astype('float32')  # to align with pytorch
        self.im = im  # For debugging only
    ...
    def update_memory(self, sample_x: TensorList, sample_y: TensorList, learning_rate=None):
        replace_ind = self.update_sample_weights(self.sample_weights, self.previous_replace_ind,
                                                 ...
                                                 self.num_init_samples, self.fparams, learning_rate)
        ...
        if self.hinge_mask is not None:
            for m, y, ind in zip(self.hinge_mask, sample_y, replace_ind):
                m[ind] = layers.cast(y >= self.params.hinge_threshold, 'float32')[0]
    ...
    def localize_target(self, scores_raw):
        ...
        for i, (sz, ksz) in enumerate(zip(self.feature_sz, self.kernel_size)):
            sf_weighted[i] = fourier.shift_fs(sf_weighted[i], math.pi * (1 - np.array([ksz[0] % 2, ksz[1] % 2]) / sz))
        ...
        translation_vec = np.reshape(disp[scale_ind].astype('float32'), [-1]) * (
        ...
        if self.params.debug >= 2:
            sz = scores.shape[-2:]
            scores = np.concatenate([scores[..., sz[0] // 2:, :], scores[..., :sz[0] // 2, :]], -2)
            scores = np.concatenate([scores[..., sz[1] // 2:], scores[..., :sz[1] // 2]], -1)
    ...
    def update_state(self, new_pos, new_scale=None):
        ...
        if new_scale is not None:
            self.target_scale = np.clip(new_scale, self.min_scale_factor, self.max_scale_factor)
            self.target_sz = self.base_target_sz * self.target_scale

        # Update pos
        inside_ratio = 0.2
        inside_offset = (inside_ratio - 0.5) * self.target_sz
        self.pos = np.maximum(np.minimum(new_pos, self.image_sz.astype('float32') - inside_offset), inside_offset)

    def get_label_function(self, sample_pos, sample_scale):
        # Generate label function
        train_y = TensorList()
        target_center_norm = (self.pos - sample_pos) / (self.img_support_sz * sample_scale)
        for sig, sz, ksz in zip(self.sigma, self.feature_sz, self.kernel_size):
            center = sz * target_center_norm + 0.5 * np.array([(ksz[0] + 1) % 2, (ksz[1] + 1) % 2], 'float32')
            train_y.append(dcf.label_function_spatial(sz, sig, center))
        return train_y

    def extract_sample(self, im: np.ndarray, pos: np.ndarray, scales, sz: np.ndarray, debug_save_name):
        return self.params.features.extract(im, pos, scales, sz, debug_save_name)

    def extract_processed_sample(self, im: np.ndarray, pos: np.ndarray, scales, sz: np.ndarray,
            ...
            if proj_init_method == 'pca':
                ...
                with fluid.dygraph.guard():
                    self.projection_matrix = TensorList(
                        [None if cdim is None else layers.gaussian_random((cdim, ex.shape[1], 1, 1), 0.0,
                                                                          1 / math.sqrt(ex.shape[1])).numpy() for
                         ex, cdim in
                         zip(x, self.compressed_dim)])
            elif proj_init_method == 'np_randn':
                rng = np.random.RandomState(0)
                self.projection_matrix = TensorList(
                    [None if cdim is None else rng.normal(size=(cdim, ex.shape[1], 1, 1),
                                                          loc=0.0,
                                                          scale=1 / math.sqrt(ex.shape[1])).astype('float32') for
                     ex, cdim in
                     zip(x, self.compressed_dim)])
            elif proj_init_method == 'ones':
                self.projection_matrix = TensorList(
                    [None if cdim is None else np.ones((cdim, ex.shape[1], 1, 1), 'float32') / math.sqrt(ex.shape[1])
                     for
                     ex, cdim in
                     zip(x, self.compressed_dim)])
        else:
    ...
    def init_label_function(self, train_x):
        ...
        self.y = TensorList(
            [np.zeros([self.params.sample_memory_size, 1, x.shape[2], x.shape[3]], 'float32') for x in train_x])

        # Output sigma factor
        output_sigma_factor = self.fparams.attribute('output_sigma_factor')
        self.sigma = output_sigma_factor * np.ones((2,), 'float32') * (
                self.feature_sz / self.img_support_sz * self.base_target_sz).apply(np.prod).apply(np.sqrt)

        # Center pos in normalized coords
        target_center_norm = (self.pos - np.round(self.pos)) / (self.target_scale * self.img_support_sz)

        # Generate label functions
        for y, sig, sz, ksz, x in zip(self.y, self.sigma, self.feature_sz, self.kernel_size, train_x):
            center_pos = sz * target_center_norm + 0.5 * np.array([(ksz[0] + 1) % 2, (ksz[1] + 1) % 2], 'float32')
            for i, T in enumerate(self.transforms[:x.shape[0]]):
                sample_center = center_pos + np.array(T.shift, 'float32') / self.img_support_sz * sz
    ...
    def init_memory(self, train_x):
        ...
        self.init_sample_weights = TensorList([np.ones(x.shape[0], 'float32') / x.shape[0] for x in train_x])
        self.init_training_samples = train_x

        # Sample counters and weights
        self.num_stored_samples = self.num_init_samples.copy()
        self.previous_replace_ind = [None] * len(self.num_stored_samples)
        self.sample_weights = TensorList([np.zeros(self.params.sample_memory_size, 'float32') for x in train_x])
        for sw, init_sw, num in zip(self.sample_weights, self.init_sample_weights, self.num_init_samples):
            sw[:num] = init_sw

        # Initialize memory
        self.training_samples = TensorList(
            [[np.zeros([cdim, x.shape[2], x.shape[3]], 'float32')] * self.params.sample_memory_size for x, cdim in
             zip(train_x, self.compressed_dim)])

    def init_learning(self):
        # Get window function
        self.feature_window = TensorList([dcf.hann2d(sz) for sz in self.feature_sz])
    ...
    def generate_init_samples(self, im: np.ndarray) -> TensorList:
        ...
        if aug_expansion_factor is not None and aug_expansion_factor != 1:
            aug_expansion_sz = (self.img_sample_sz * aug_expansion_factor).astype('long')
            aug_expansion_sz += (aug_expansion_sz - self.img_sample_sz.astype('long')) % 2
        ...
        if random_shift_factor > 0:
            get_rand_shift = lambda: (
                    (np.random.uniform(size=[2]) - 0.5) * self.img_sample_sz * random_shift_factor).astype(
                'long').tolist()
        ...
        if 'blur' in self.params.augmentation:
            self.transforms.extend(
                [augmentation.Blur(sigma, aug_output_sz, get_rand_shift()) for sigma in
        ...
        if 'rotate' in self.params.augmentation:
            self.transforms.extend(
                [augmentation.Rotate(angle, aug_output_sz, get_rand_shift()) for angle in
                ...
                for i, use_aug in enumerate(self.fparams.attribute('use_augmentation')):
                    if use_aug:
                        init_samples[i] = np.concatenate(
                            [init_samples[i],
                             dropout2d(layers.expand(n2p(init_samples[i][0:1]), (num, 1, 1, 1)),
                                       prob,
                                       is_train=True).numpy()])
    ...
    def init_optimization(self, train_x, init_y):
        ...
        if filter_init_method == 'zeros':
            ...
            for idx, f in enumerate(self.filter):
                self.filter[idx] = np.ones(f.shape, 'float32') / np.prod(f.shape)
        elif filter_init_method == 'np_randn':
            rng = np.random.RandomState(0)
            for idx, f in enumerate(self.filter):
                self.filter[idx] = rng.normal(size=f.shape,
                                              loc=0,
                                              scale=1 / np.prod(f.shape)).astype('float32')
        elif filter_init_method == 'randn':
            for idx, f in enumerate(self.filter):
                with fluid.dygraph.guard():
                    self.filter[idx] = layers.gaussian_random(f.shape, std=1 / np.prod(f.shape)).numpy()
        else:
        ...
        if self.params.update_projection_matrix:
            ...
            if optimizer == 'GaussNewtonCG':
                ...
                self.joint_optimizer = GradientDescentL2(self.joint_problem, joint_var,
                                                         ...
                                                         debug=analyze_convergence, fig_num=(12, 13))
            ...
            self.projection_matrix = self.joint_optimizer.x[len_x // 2:]  # w1 in paper
    ...
    def project_sample(self, x: TensorList, proj_matrix=None):
        ...
        with fluid.dygraph.guard():
            return operation.conv2d(x.apply(n2p), proj_matrix.apply(n2p)).apply(self.projection_activation).numpy()
    ...
    def init_iou_net(self):
        ...
        if self.params.iounet_augmentation:
            for T in self.transforms:
                if not isinstance(T, (
                        ...
                        augmentation.Blur)):
                    break
                target_boxes.append(self.iou_target_box + np.array([T.shift[1], T.shift[0], 0, 0]))
        else:
        ...
        with fluid.dygraph.guard():
            ...
            self.target_feat = TensorList([layers.reduce_mean(x, 0).numpy() for x in target_feat])

        if getattr(self.params, 'iounet_not_use_reference', False):
            self.target_feat = TensorList([np.full_like(tf, tf.norm() / tf.numel()) for tf in self.target_feat])
    ...
    def optimize_boxes(self, iou_features, init_boxes):
        with fluid.dygraph.guard():
            ...
            for i_ in range(self.params.box_refinement_iter):
                ...
                outputs = self.iou_predictor.predict_iou(target_feat, iou_features, bb_init)
            ...
            return layers.reshape(output_boxes, (-1, 4)).numpy(), layers.reshape(outputs, (-1,)).numpy()
    ...
    def refine_target_box(self, sample_pos, sample_scale, scale_ind, update_scale=True):
        ...
        if self.params.num_init_random_boxes > 0:
            ...
            rand_factor = square_box_sz * np.concatenate(
                [self.params.box_jitter_pos * np.ones(2), self.params.box_jitter_sz * np.ones(2)])
            minimal_edge_size = init_box[2:].min() / 3
            rand_bb = (rand_fn(self.params.num_init_random_boxes, 4) - 0.5) * rand_factor
            new_sz = np.clip(init_box[2:] + rand_bb[:, 2:], minimal_edge_size, 1e10)
            new_center = (init_box[:2] + init_box[2:] / 2) + rand_bb[:, :2]
            init_boxes = np.concatenate([new_center - new_sz / 2, new_sz], 1)
            init_boxes = np.concatenate([np.reshape(init_box, (1, 4)), init_boxes])
        ...
        predicted_iou = np.mean(np.reshape(output_iou, (-1, 1))[inds, :], axis=0)
        ...
        new_scale = np.sqrt(np.prod(new_target_sz) / np.prod(self.base_target_sz))
    ...
    def localize_advanced(self, scores):
        ...
        # Shift scores back
        scores = np.concatenate([scores[..., (sz[0] + 1) // 2:, :], scores[..., :(sz[0] + 1) // 2, :]], -2)
        scores = np.concatenate([scores[..., :, (sz[1] + 1) // 2:], scores[..., :, :(sz[1] + 1) // 2]], -1)
        ...
        tneigh_bottom = int(min(round(max_disp1[0] + target_neigh_sz[0] / 2 + 1), sz[0]))
        tneigh_left = int(max(round(max_disp1[1] - target_neigh_sz[1] / 2), 0))
        tneigh_right = int(min(round(max_disp1[1] + target_neigh_sz[1] / 2 + 1), sz[1]))
