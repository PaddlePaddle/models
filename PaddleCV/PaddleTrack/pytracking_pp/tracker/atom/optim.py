...
from pytracking_pp.libs import optimization, TensorList, operation
from pytracking_pp.libs.paddle_utils import PTensor, broadcast_op, n2p, static_identity
import math
...
class FactorizedConvProblem(optimization.L2Problem):
    def __init__(self, training_samples: TensorList, y: TensorList, filter_reg: TensorList, projection_reg, params,
    ...
    def get_inputs(self, scope=''):
        if scope not in self.inputs_dict:
            training_samples_p = TensorList([fluid.layers.data('{}training_samples_{}'.format(scope, idx),
                                                               shape=[None] + list(v[0].shape),
                                                               stop_gradient=False, append_batch_size=False) for
                                             idx, v in enumerate(self.training_samples)])
            y_p = TensorList([fluid.layers.data('{}y_{}'.format(scope, idx), shape=[None] + list(v[0].shape),
                                                stop_gradient=False, append_batch_size=False) for idx, v in
                              enumerate(self.y)])
            sample_weights_p = TensorList([fluid.layers.data('{}sample_weights_{}'.format(scope, idx),
                                                             shape=[None, 1], stop_gradient=False,
                                                             append_batch_size=False) for idx, v in
                                           enumerate(self.sample_weights)])
            self.inputs_dict[scope] = (training_samples_p, y_p, sample_weights_p)
    ...
    def get_feed_dict(self, scope=''):
        if self.training_samples_stack is None or self.y_stack is None or self.sample_weights_stack is None:
            self.training_samples_stack = self.training_samples.apply(stack_input)
    ...
    def __call__(self, x: TensorList, scope=''):
        ...
        compressed_samples = operation.conv1x1(training_samples, P).apply(self.projection_activation)
        ...
        residuals.extend(filter.apply(static_identity) * self.filter_reg.apply(math.sqrt))

        # Add regularization for projection matrix
        residuals.extend(P.apply(static_identity) * self.projection_reg.apply(math.sqrt))
...
class ConvProblem(optimization.L2Problem):
    def __init__(self, training_samples: TensorList, y: TensorList, filter_reg: TensorList, sample_weights: TensorList,
                 response_activation):
        self.training_samples = training_samples
        self.y = y
        self.filter_reg = filter_reg
    ...
    def get_inputs(self, scope=''):
        if scope not in self.inputs_dict:
            ...
            y_p = TensorList([fluid.layers.data('{}y_{}'.format(scope, idx), shape=[None] + list(v[0].shape),
                              ...
                              enumerate(self.y)])
            sample_weights_p = TensorList([fluid.layers.data('{}sample_weights_{}'.format(scope, idx),
                                                             shape=[None] + list(v[0].shape), stop_gradient=False,
                                                             append_batch_size=False) for idx, v in
                                           enumerate(self.sample_weights)])
            self.inputs_dict[scope] = (training_samples_p, y_p, sample_weights_p)
    ...
    def __call__(self, x: TensorList, scope=''):
        ...
        residuals = operation.conv2d(training_samples, x, mode='same').apply(self.response_activation)
        residuals = residuals - y

        residuals = residuals * samples_weights.sqrt()

        # Add regularization for projection matrix
        residuals.extend(x.apply(static_identity) * self.filter_reg.apply(math.sqrt))
