...
class ResNet18(MultiFeatureBase):
    ...
    def __init__(self, output_layers=('block2',), net_path='atom_iou', use_gpu=True, *args, **kwargs):
    ...
    def initialize(self):
        with fluid.dygraph.guard():
            ...
            self.net = atom_resnet18(backbone_pretrained=False, backbone_is_test=True, iounet_is_test=True)
        ...
        self.layer_stride = {'conv0': 2, 'conv1': 2, 'block0': 4, 'block1': 8, 'block2': 16, 'block3': 32,
                             'classification': 16,
                             'fc': None}
        self.layer_dim = {'conv0': 64, 'conv1': 64, 'block0': 64, 'block1': 128, 'block2': 256, 'block3': 512,
    ...
    def extract(self, im: np.ndarray, debug_save_name=None):
        with fluid.dygraph.guard():
            ...
            self.iounet_features = TensorList(
                [f.numpy() for f in self.iou_predictor.get_iou_feat(iounet_backbone_features)])

            output = TensorList([output_features[layer].numpy() for layer in self.output_layers])
            return output
...
class ResNet50(MultiFeatureBase):
    ...
    def initialize(self):
        with fluid.dygraph.guard():
            ...
            self.net = atom_resnet50(backbone_pretrained=False, backbone_is_test=True, iounet_is_test=True)
        ...
        self.layer_stride = {'conv0': 2, 'conv1': 2, 'block0': 4, 'block1': 8, 'block2': 16, 'block3': 32,
                             'classification': 16,
                             'fc': None}
        self.layer_dim = {'conv0': 64, 'conv1': 64, 'block0': 256, 'block1': 512, 'block2': 1024, 'block3': 2048,
                          'classification': 256, 'fc': None}
...
class SFCAlexnet(MultiFeatureBase):
    ...
    def __init__(self, output_layers=('conv5',), net_path='estimator', use_gpu=True, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def initialize(self):
        with fluid.dygraph.guard():
            ...
            self.net = siamfc_alexnet(backbone_pretrained=False, backbone_is_test=True, estimator_is_test=True)
        ...
        self.feature_layers = sorted(list(set(self.output_layers + self.estimator_feature_layers)))
