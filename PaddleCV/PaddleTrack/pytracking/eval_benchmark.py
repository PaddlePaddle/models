...
parser.add_argument('--training_base_param', '-tr', type=str, help='training base params name')
parser.add_argument('--epoch', '-e', type=str, help='epoch specifications')
parser.add_argument('--tracking_base_param', '-te', type=str, help='tracking base params name')
parser.add_argument('--num_repeat', '-n', default=1, type=int, help='number of repeat')
parser.add_argument('--exp_id', '-ex', default='', type=str, help='experiment id')
...
def create_tracker(params):
    ...
    param_module = importlib.import_module('pytracking_pp.parameter.{}'.format(base_param))
    tracker_params = get_tracker_params(param_module, params)
    tracker_module = importlib.import_module('pytracking_pp.tracker.{}'.format(base_tracker))
...
def compute_evaluation_metrics(dataset, params):
    result_dir = osp.join(params['result_dir'], params['save_dataset_name'], params['tracking_base_param'])
...
def run_tracking_and_evaluate(params):
    ...
    # load dataset
    root = os.path.abspath(osp.join(env_settings().dataset_path, params['save_dataset_name']))
...
def get_checkpoint_path(training_base_param, epoch):
    model_dir = osp.abspath(osp.join(env_settings().network_path, *training_base_param.split('.')))
