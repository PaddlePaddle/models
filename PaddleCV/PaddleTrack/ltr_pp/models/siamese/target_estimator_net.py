...
from pytracking_pp.libs.Fconv2d import Conv2D


class SiamFCEstimator(dygraph.layers.Layer):
    def __init__(self, name):
        super().__init__(name)
        init_w = fluid.ParamAttr(name="a_weight", initializer=fluid.initializer.ConstantInitializer(0.001),
                                 learning_rate=0., trainable=False)
        init_b = fluid.ParamAttr(name="a_bias", initializer=fluid.initializer.ConstantInitializer(0.), trainable=True)

        self.adjust_conv = nn.Conv2D(1, 1, 1, 1, 0, param_attr=init_w, bias_attr=init_b)
    ...
    def estimate(self, exemplar, instance):
        ...
        instance = fluid.layers.reshape(instance, shape=[1, -1, shape[2], shape[3]])
