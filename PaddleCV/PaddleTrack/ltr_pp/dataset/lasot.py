...
class Lasot(BaseDataset):
    ...
    def __init__(self, root=None, filter=None, image_loader=default_image_loader, vid_ids=None, split=None):
        """
        args:
    ...
    def _read_target_visible(self, seq_path, anno):
        ...
        with open(occlusion_file, 'r', newline='') as f:
            occlusion = np.array([int(v) for v in list(csv.reader(f))[0]], 'byte')
        with open(out_of_view_file, 'r') as f:
            out_of_view = np.array([int(v) for v in list(csv.reader(f))[0]], 'byte')
