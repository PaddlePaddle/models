    ...
    with fluid.program_guard(main_prog, startup_prog):
        with fluid.unique_name.guard():
            if is_train and use_mixup:
                ...
                if args.fp16:
                    image = fluid.layers.cast(image, "float16")
                avg_cost = net_config(image=image,model=model, args=args,is_train=is_train,label=0, y_a=y_a, y_b=y_b, lam=lam)
                avg_cost.persistable = True
                build_program_out = [py_reader, avg_cost]
            else:
                py_reader = fluid.layers.py_reader(
                ...
                if args.fp16:
                    image = fluid.layers.cast(image, "float16")
                avg_cost, acc_top1, acc_top5 = net_config(image=image,model=model, args=args, label=label, is_train=is_train)
                acc_top1.persistable = True
                acc_top5.persistable = True
                avg_cost.persistable = True
                build_program_out = [py_reader, avg_cost, acc_top1, acc_top5]
