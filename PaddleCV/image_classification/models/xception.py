...
class Xception(object):
    ...
    def entry_flow(self, input, block_num=3):
        ...
        conv = self.conv_bn_layer(
            input=input, num_filters=32, filter_size=3, stride=2, act='relu', name=name+"_conv1")
        conv = self.conv_bn_layer(
            input=conv, num_filters=64, filter_size=3, stride=1, act='relu', name=name+"_conv2")
        ...
        for block in range(block_num):
            ...
            conv = self.entry_flow_bottleneck_block(conv, 
    ...
    def entry_flow_bottleneck_block(self, input, num_filters, name, stride=2, relu_first=False):
        ...
        conv1 = self.separable_conv( conv0, num_filters, stride=1, name=name+"_branch2a_weights" )
        
        conv2 = fluid.layers.relu( conv1 )
        conv2 = self.separable_conv( conv2, num_filters, stride=1, name=name+"_branch2b_weights" )
    ...
    def middle_flow_bottleneck_block(self, input, num_filters, name):
        ...
        conv0 = self.separable_conv( conv0, num_filters=num_filters, stride=1, name=name+"_branch2a_weights" )
        
        conv1 = fluid.layers.relu( conv0 )
        conv1 = self.separable_conv( conv1, num_filters=num_filters, stride=1, name=name+"_branch2b_weights" )
        
        conv2 = fluid.layers.relu( conv1 )
        conv2 = self.separable_conv( conv2, num_filters=num_filters, stride=1, name=name+"_branch2c_weights" )
    ...
    def exit_flow(self, input, class_dim):
        ...
        num_filters1 = 728
        num_filters2 = 1024
        conv0 = self.exit_flow_bottleneck_block( input, num_filters1, num_filters2, name=name+"_1" )
        
        conv1 = self.separable_conv( conv0, num_filters=1536, stride=1, name=name+"_2" )
        conv1 = fluid.layers.relu( conv1 )
        
        conv2 = self.separable_conv( conv1, num_filters=2048, stride=1, name=name+"_3" )
    ...
    def exit_flow_bottleneck_block(self, input, num_filters1, num_filters2, name):
        '''entry_flow_bottleneck_block'''
        short = fluid.layers.conv2d(
            input=input,
        ...
        conv0 = fluid.layers.relu( input )
        conv1 = self.separable_conv( conv0, num_filters1, stride=1, name=name+"_branch2a_weights" )
        
        conv2 = fluid.layers.relu( conv1 )
        conv2 = self.separable_conv( conv2, num_filters2, stride=1, name=name+"_branch2b_weights" )
